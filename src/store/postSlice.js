...
export const createPost = createAsyncThunk(
  ...
  async (postData, { rejectWithValue }) => {
    try {
      ...
      if (postData.categories) formData.append('categories', postData.categories);
      if (postData.tags) formData.append('tags', postData.tags);
      if (postData.isPublished !== undefined) formData.append('isPublished', postData.isPublished);
...
export const getAllPosts = createAsyncThunk(
  'posts/getAll',
  async ({ page = 1, limit = 10, category, published }, { rejectWithValue }) => {
    try {
...
export const getRelatedPosts = createAsyncThunk(
  ...
  async (postId, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Failed to fetch related posts');
    }
  }
);
...
export const searchPosts = createAsyncThunk(
  'posts/search',
  async ({ search, tag, category, page = 1, limit = 10 }, { rejectWithValue }) => {
    try {
      const params = { search, page, limit };
      if (tag) params.tag = tag;
      if (category) params.category = category;

      const response = await axiosInstance.get('/post/filtersearch', { params });
      return response.data;
    } catch (error) {
